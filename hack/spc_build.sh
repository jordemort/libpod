...
git remote add upstream github:projectatomic/libpod.git
...
if [ $SHORTCOMMIT == $CURRENT_SHORTCOMMIT ]; then
    ...
    if [ $VERSION == $CURRENT_VERSION ]; then
        ...
        debchange --package "podman" -v "$VERSION-1~ubuntu16.04.2~ppa1" -D xenial "bump to $VERSION, autobuilt $SHORTCOMMIT"
        git commit -asm "bump to $VERSION, autobuilt $SHORTCOMMIT"
    fi

    echo "Substituting GIT_COMMIT in debian/rules to make deb build happy..."
    sed -i "s/\${GIT_COMMIT}/$SHORTCOMMIT/" debian/rules
    ...
    echo "Local build success, building package for PPA submission..."
